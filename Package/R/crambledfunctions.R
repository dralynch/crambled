...
CrambledScan<-function(normalbam,tumourbam,title,window=51,redline=F,...){
  ...
  #test<-require(Rsamtools)
    ...
    fls <- Rsamtools::PileupFiles(c(normalbam, tumourbam))
    param <- Rsamtools::ApplyPileupsParam(which=mywhich, what="seq")
    res <- Rsamtools::applyPileups(fls, CrambledScanInfo, param=param)
...
CrambledScanCellline<-function(celllinebam,title,window=51,AFthreshold=0.1,...){
  ...
  #test<-require(Rsamtools)
    ...
    ## Use Rsamtools to pull out the depths and allele fractions at those locations
    fls <- Rsamtools::PileupFiles(celllinebam)
    param <- Rsamtools::ApplyPileupsParam(which=mywhich, what="seq")
    res <- Rsamtools::applyPileups(fls, CrambledScanInfoCellline, param=param)
...
CrambledPlot<-function(depthvec,afvec,title,xlimmin=0,xlimmax=100,redline=F,xpower=1){
  ...
  png(width=600,height=400,filename=paste(title,"-shiny.png",sep="",collapse=""))
...
CrambledSuggestSNPsHG19<-function(){
  ...
  if (appDir == "") {
    stop("Could not find Data directory. Try re-installing `Crambled`.", call. = FALSE)
  }
  
 if(!exists(file.path(appDir,"CrambledGRangesHG19"))){
   CGHG19<-readRDS(file.path(appDir,"CrambledGRangesHG19.rds"))   
 }
 return(CGHG19)  
}
